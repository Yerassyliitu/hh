...
from src.services.resume_offer import ResumeOfferService

worker_resume_offer_router = APIRouter(prefix="/v1/worker/resume/offers", tags=["worker / resume / offers"])


@worker_resume_offer_router.get(
    "/",
    status_code=200,
    summary="Get a list of resume-offers for user(то есть заявки которые оставил юзер)"
)
async def get_resume_offers(
    resume_offer_service: Annotated[ResumeOfferService, Depends(resume_service)],
    user: Annotated[dict, Depends(get_current_user)]
):
    resume_offers = await resume_offer_service.get_entities(user_id=user["id"])
